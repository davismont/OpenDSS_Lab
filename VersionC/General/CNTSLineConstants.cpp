...
int TCNTSLineConstants::Get_CondType(int i)
{
    int result = 0;
    result = FkStrand[i - 1];
    return result;
...
void TCNTSLineConstants::Set_CondType(int i, int Value)
{
    if((i > 0) && (i <= FNumConds))
        FCondType[i - 1] = Value;
}
...
int TCNTSLineConstants::Get_kStrand(int i)
{
	int result = 0;
	result = FkStrand[i - 1];
	return result;
...
double TCNTSLineConstants::Get_DiaStrand(int i, int Units)
{
	double result = 0.0;
...
double TCNTSLineConstants::Get_GmrStrand(int i, int Units)
{
	double result = 0.0;
...
double TCNTSLineConstants::Get_RStrand(int i, int Units)
{
	double result = 0.0;
...
bool TCNTSLineConstants::Get_Semicon(int i)
{
	bool result = true;
	result = FSemicon[i - 1];
	return result;
...
void TCNTSLineConstants::Set_kStrand(int i, int Value)
{
	if((i > 0) && (i <= FNumConds))
		FkStrand[i - 1] = Value;
}

void TCNTSLineConstants::Set_DiaStrand(int i, int Units, double Value)
{
	if((i > 0) && (i <= FNumConds))
		FDiaStrand[i - 1] = Value * To_Meters(Units);
}

void TCNTSLineConstants::Set_GmrStrand(int i, int Units, double Value)
{
	if((i > 0) && (i <= FNumConds))
		FGmrStrand[i - 1] = Value * To_Meters(Units);
}

void TCNTSLineConstants::Set_RStrand(int i, int Units, double Value)
{
	if((i > 0) && (i <= FNumConds))
		FRStrand[i - 1] = Value * To_per_Meter(Units);
}

void TCNTSLineConstants::Set_Semicon(int i, bool Value)
{
	if((i > 0) && (i <= FNumConds))
		FSemicon[i - 1] = Value;
}
...
void TCNTSLineConstants::Calc(double f)
{
	complex Zi = {};
	complex Zspacing = {};
...
TCNTSLineConstants::TCNTSLineConstants(int NumConductors)
