...
begin
      ...
      For i := 1 to FnumConds Do Begin
          ...
          begin
            if (i > FNumPhases) then FYCMatrix.SetElement(i, i, cmplx(0.0, pfactor * ln(2.0*FEqDist[4 - 1]/Fcapradius^[i])))
            else FYCMatrix.SetElement(i, i, cmplx(0.0, pfactor * ln(2.0*FEqDist[3 - 1]/Fcapradius^[i])));
          end;
      End;
      ...
      For i := 1 to FNumConds Do Begin
        For j := 1 to i-1 Do Begin
            ...
            begin
              ...
              if ((j <= FNumPhases) and (i > FNumPhases)) then Dijp := (FEqDist[3 - 1] + FEqDist[4 - 1]) // AvgHeightPhase + AvgHeightNeutral
              else if ((i <= FNumPhases) and (j <= FNumPhases)) then Dijp := (2 * FEqDist[3 - 1]) // 2 * AvgHeightPhase
...
{Check all conductors to make sure none occupy the same space or are defined at 0,0}
     ...
     begin
       ...
       if (FEqDist[3 - 1] <= 0.0) or (FEqDist[4 - 1] <= 0.0) then Begin
           Result := TRUE;
       ...
       For i := 1 to FNumConds do Begin
         for j := i+1 to FNumConds do Begin
           if ((i <= FNumPhases) and (j > FNumPhases)) then Dij := FEqDist[2 - 1]
...
begin
    ...
    else if i <= FNumPhases then Fyi := Abs(FEqDist[3 - 1])
    else Fyi := Abs(FEqDist[4 - 1]);

    if not FEquivalentSpacing then Fyj := Abs(Fy^[j])
    else if j <= FNumPhases then Fyj := Abs(FEqDist[3 - 1])
    ...
    else if ((i <= FNumPhases) and (j <= FNumPhases)) or ((i > FNumPhases) and (j > FNumPhases)) then Fxi_Fxj := FEqDist[1 - 1]
