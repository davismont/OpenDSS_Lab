  ...
  Begin
    ...
    if FPPriority and ((FReacPower_ref = 'VARMAX') or (ControlMode = WATTPF)) then
      begin
        ...
        if abs(Qavailable_sqr) < EPSILON then Qavailable_sqr := 0;

        if Q >= 0.0 then Q_Ppriority :=  Sqrt(Qavailable_sqr)/QHeadRoom
        else Q_Ppriority :=  Sqrt(Qavailable_sqr)/QHeadRoomNeg;
